...
5. Make sure the database is not exposed publicly. Use `docker-compose.prod.yml` or a private managed PostgreSQL instance.
6. Put the app behind Nginx with HTTPS enabled. Use `nginx/nginx.conf` as the base.
...
10. Run `npm run env:check` before every production deploy.
